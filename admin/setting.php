...
$query = "SELECT name, username, mobile_number, password FROM systemadmin WHERE id = $1";
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['update'])) {
     ...
     if (empty($name) || empty($username) || empty($mobile_number) || empty($password)) {
          ...
          if (!password_verify($password, $row['password'])) {
               ...
               $update_query = "UPDATE systemadmin SET name = $1, username = $2, mobile_number = $3 WHERE id = $4";
               ...
               <?php if ($row) { ?>
                    <form action="setting.php" method="POST">
                         ...
                         <input type="text" name="mobile_number" maxlength="10" pattern="\d{10}"
                              value="<?php echo htmlspecialchars($row['mobile_number']); ?>" required><br><br>
