...
if ($id) {
     $sqlEdit = "SELECT * FROM district_admins WHERE id = $1";
     ...
     if ($_SERVER["REQUEST_METHOD"] == "POST") {
          ...
          // if (password_verify($password, $stored_password)) {
               ...
               $query = "UPDATE district_admins SET name=$1, username=$2,  mobile_number=$3, otp=$4, status=$5::BOOlean, district_name=$6 WHERE id=$7";

               $updateResult = pg_query_params($fsms_conn, $query, [$name, $username, $mobile_number, $otp, $status, $district_name, $id]);

               if ($updateResult) {
                    echo "<script>alert('You have updated a District admin'); window.location.href = 'dashboard.php'</script>";
               } else {
