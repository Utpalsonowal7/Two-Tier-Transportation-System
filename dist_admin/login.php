...
if (isset($_POST['login'])) {
     ...
     $query = "SELECT * FROM district_admins WHERE username = $1";
     ...
     if (pg_num_rows($result) > 0) {
          ...
          if ($num['status'] === 'f' || $num['status'] === false) {
               echo "<script>alert('Your account is inactive. Contact the system admin.'); window.location.href='login.php';</script>";
               exit();
          }
          ...
          if (password_verify($pass, $num['password'])) {
               ...
               pg_query_params($fsms_conn, "UPDATE district_admins SET otp = $1 WHERE id = $2", array($generateOtp, $num['id']));
               // echo "<script>window.location.href='dist_dashboard.php';</script>";
               // exit();
               $showOtpModal = true;
          } else {
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['otp'])) {
     ...
     $res = pg_query_params($fsms_conn, "SELECT otp FROM district_admins WHERE username=$1", array($adminusername));
...
<head>
     <meta charset="utf-8">
     <title>Dist Admin Login Form </title>
     	<link rel="shortcut icon" href="../assets/favicon.png" type="image/x-icon">
          <meta name="description" content="Login form for district admin">
