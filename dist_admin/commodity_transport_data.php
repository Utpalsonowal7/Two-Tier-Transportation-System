...
if ($show_data) {
    $query = "SELECT warehouse_name, wholesaler_name, commodity, quantity, rate, distance, transport_date
              FROM transport 
              WHERE transport_date BETWEEN '$start_date_time' AND '$end_date_time'
...
<body>
    <div class="container">
        ...
        <h2>Tier 1 Monthly Transport Report</h2>
            ...
            <table id="report-table" class="display nowrap" style="width:100%">
                ...
                <tbody>
                    ...
                    while ($row = pg_fetch_assoc($result)) {
                        ...
                        $total = $quantity * $rate;
                        $grand_total += $total;
                        $created_at = date("Y-m-d", strtotime($row['transport_date']));
        ...
        $(document).ready(function () {
            window.table = $('#report-table').DataTable({
                ...
                buttons: [
                    {
                        extend: 'excelHtml5',
                        title: 'Monthly Transport Report Tier 1 from <?php echo $start_date . " to " . $end_date; ?>',
                        footer: true,
